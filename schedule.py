...
class Stadium:
    ...
    def __init__(self, name:str, x:double, y:double, geolocation:str):
        self.name = name
        self.x = x
        self.y = y
...
class Tournament:
    ...
    def _init_locations(stadiums : list[Stadium]):
        res = {}
        for s in stadiums:
    ...
    def home_away_swap(self, round:int, slot:int, schedule = None):
        # swap home and away
        new_schedule = schedule if schedule is not None else self.schedule.deepcopy()
        ...
        new_schedule[round][slot][2] = self.team[new_home_team].homeidx

        return new_schedule

    def swap_timeslot(self, r1:int, r2:int, ts1:int, ts2:int, schedule = None):
        new_schedule = schedule if schedule is not None else self.schedule.deepcopy()
        new_schedule[r1][ts1], new_schedule[r2][ts2] = new_schedule[rs2][ts2], new_schedule[ts1][r1]
