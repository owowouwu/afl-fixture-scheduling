...
def generate_initial_fixture():
    ...
    # Create a new model
    model = gp.Model("fixture_scheduling")
    ...
    for i in Ts: # Each team plays once a week
        for r in rounds:
            model.addConstr(gp.quicksum(fixture[i, j, s, t, r] + fixture[j, i, s, t, r] for j in Ts for s in Ss for t in timeslots) == 1, "MatchesEachRound")
            
    
    for i in Ts: # 11 Home Games 
        model.addConstr(gp.quicksum(fixture[i, j, stadium_numbers[s], t, r] for j in Ts for s in home_stadiums[i] 
                                 for t in timeslots for r in rounds) == 11, f"HomeGames_{i}")
    
    
    for i in Ts:
        model.addConstr(gp.quicksum(fixture[i, i, s, t, r] for s in Ss for t in timeslots for r in rounds) <= 0, f"DoNotPlaySelf")
        for j in Ts:
            if i != j:
                model.addConstr(gp.quicksum(fixture[i, j, s, t, r] for s in Ss for t in timeslots for r in rounds) <= 1, f"PlayMoreThanTwice_{i}_{j}")
                model.addConstr(gp.quicksum(fixture[i, j, s, t, r] + fixture[j, i, s, t, r] for s in Ss for t in timeslots for r in rounds) >= 1, f"PlayAtLeastOnce_{i}_{j}")
                
                
    for i in Ts:
        for r in rounds[:-1]:
            model.addConstr(1 >= gp.quicksum(fixture[i, j, s, t, r]+fixture[j, i, s, t, r] for j in Ts for s in Ss for t in [5,6]) + 
                            gp.quicksum(fixture[i, j, s, t, r+1]+fixture[j, i, s, t, r+1] for j in Ts for s in Ss for t in [0]),
    ...
    for i in Ts:
        for r in rounds[:-2]:
            model.addConstr(gp.quicksum(fixture[j, i, stadium_numbers[s], t, r_]+fixture[i,j, stadium_numbers[s], t, r_] for j in Ts for s in home_location_stadiums[i] 
                                 for t in timeslots for r_ in range(r,r+3)) >= 1, f"ThreeGamesInRowOutside_{i}_{r}")
       
    # Four away games in a row
    for i in Ts:
        for r in rounds[:-3]:
            model.addConstr(gp.quicksum(fixture[i, j, s, t, r_] for j in Ts for s in Ss for t in timeslots
    ...
    for r in rounds:
        for s in Ss:
            model.addConstr(gp.quicksum(fixture[i, j, s, t, r] for i in Ts for j in Ts for t in [5, 6]) <= 1, f"SundayGamesInStadium_{r}_{s}")
            
            model.addConstr(gp.quicksum(fixture[i, j, s, t, r] for i in Ts for j in Ts for t in [2, 3, 4]) <= 1, f"SaturdayGamesInStadium_{r}_{s}")
            
            for t in [0,1]:
                model.addConstr(gp.quicksum(fixture[i, j, s, t, r] for i in Ts for j in Ts) <= 1, f"OneGameAtATimeInStadium_{r}_{s}")
    
    
    # Constraint: No more than two games in any timeslot, and only one on Thursday and Friday night
    for r in rounds:
        
        model.addConstr(gp.quicksum(fixture[i, j, s, t, r] for i in Ts for j in Ts for s in Ss for t in [5,6]) >= 2, f"AtLeastTwoSundayGames")
        
        for t in [0,1]:
            model.addConstr(gp.quicksum(fixture[i, j, s, t, r] for i in Ts for j in Ts for s in Ss) <= 1, f"OneThursday&FridayNightGame_{r}")
            
        for t in [2,3,4,5,6]:
            model.addConstr(gp.quicksum(fixture[i, j, s, t, r] for i in Ts for j in Ts for s in Ss) <= 2, f"NoMoreThanTwoSimultaneousGames_{r}")
        
        for t in timeslots:
            model.addConstr(gp.quicksum(fixture[i, j, s, t, r] for i in Ts for j in Ts for s in Ss) >= game_on[t,r], f"IncentiviseAtLeastOneGameInEachTimeslot_{r}_{t}")
            
    
           
    for i in Ts:
        model.addConstr(equality[i] >= gp.quicksum(probability_win(i, j, s)*fixture[i,j,s,t,r] + (1-probability_win(j,i,s))*fixture[j,i,s,t,r] 
                                                   for j in Ts for s in Ss for t in timeslots for r in rounds)-11)
                
        model.addConstr(equality[i] >= 11-gp.quicksum(probability_win(i, j, s)*fixture[i,j,s,t,r] + (1-probability_win(j,i,s))*fixture[j,i,s,t,r] 
