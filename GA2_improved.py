...
def genesis():
    ...
    # MIP_soln1 = np.load('initialPopulation\MILP_Fixture_10000_2022.npy')
    # MIP_soln2 = np.load('initialPopulation\MILP_Fixture_1000000_2022.npy')
    # Greedy_soln1 = np.load('initialPopulation/2022-greedy1-1.npy')
...
def evolutionPlus():
    ...
    while time.time() - start_time < duration:  
        ...
        sorted(pop, key=lambda x: (x[2] + x[3], -x[1]))
        solns.append(best_soln)
        
    np.save("GA_soln_improved_progress.npy", solns)
...
np.save('GA_soln_improved.npy', [best_soln, num_gen])
